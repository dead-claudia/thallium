...
function formatArray(ctx, value, recurseTimes, visibleKeys, keys) {
    ...
    return output.concat(
        keys.filter(key => !key.match(/^\d+$/)).map(key =>
...
const dateToUTCString = Date.prototype.toUTCString
...
const symbolToString = typeof global.Symbol === "function" &&
...
function reduceToSingleString(output, base, [start, end]) {
    let numLinesEst = 0
    ...
    if (length > 60) {
        if (base !== "") base += "\n "
        return `${start}${base} ${output.join(",\n  ")} ${end}`
    }

    return `${start}${base} ${output.join(", ")} ${end}`
}
...
function _extend(origin, add) {
    ...
    Object.keys(add).forEach(key => origin[key] = add[key])
