...
t.test 'cli common resolveDefault() gets ES6 default objects with `default` property', ->
t.test 'cli common resolveDefault() gets ES6 default functions with `default` property', ->
t.test 'cli common resolveDefault() gets ES6 default arrays with `default` property', ->
...
t.test 'cli common normalizeGlob() edge cases normalizes `..` with a cwd of `.`', ->
t.test 'cli common normalizeGlob() edge cases normalizes `.` with a cwd of `..`', ->
t.test 'cli common normalizeGlob() edge cases normalizes directories with a cwd of `..`', ->
...
t.test 'cli common globParent() gets a base name from brace .. expansion', ->
t.test 'cli common globParent() gets a base name from extglob', ->
t.test 'cli common globParent() gets a base name from a complex brace glob', ->
