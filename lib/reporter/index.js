...
var SupportsColor = Console.SupportsColor
...
exports.on = function (methods) {
    return function (opts) {
        var reporter = new Reporter(opts, methods)
        ...
        return function (ev) {
            ...
            if (!SupportsColor.forced && reporter.opts.color != null) {
                SupportsColor.set(reporter.opts.color)
            }
...
function defaultify(r, opts, accepts, prop, promisify) { // eslint-disable-line max-params, max-len
    if (accepts.indexOf(prop) >= 0) {
        var used
 ...
 */
function Reporter(opts, methods) {
    if (opts == null) opts = {}
    if (isReport(opts)) {
    ...
    if (!SupportsColor.forced && methods.accepts.indexOf("color") >= 0) {
        this.opts.color = opts.color
    }

    defaultify(this, opts, methods.accepts, "print", promisify1)
    defaultify(this, opts, methods.accepts, "write", promisify1)
    defaultify(this, opts, methods.accepts, "reset", promisify0)
    if (typeof methods.init === "function") this.init = methods.init
    else this.init = function () {}
    this.reset()
}
