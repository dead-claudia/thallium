...
methods(Reporter, {
    ...
    get: function (path) {
        if (isRepeat(this._cache, path)) {
            return this._cache.result
        }

        var child = this._base
        ...
        return this._cache.result = child
    },
    ...
    printResults: function () {
        ...
        if (!this.tests && !this.skip) {
            return this.print(color("plain", "  0 tests"))
            .then(function () { return self.print() })
        }

        return this.print()
        .then(function () {
            var tests = self.tests === 1 ? "1 test" : self.tests + " tests"

            return self.print(color("plain", "  " + tests))
        })
        .then(function () {
