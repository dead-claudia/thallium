...
function asyncFail(state, e) {
    try {
        ...
        return asyncFinish(state, tryFail(e))
    }
}
...
function asyncCallback(state, err) {
    ...
    if (state.count++) {
        ...
        // Ignore errors here.
        try2(report, undefined, state.test, r(Types.Extra,
            {count: state.count, value: err, stack: stack},
            -1))
    } else if (err != null) {
        asyncFail(state, err)
    } else {
...
function checkSpecial(state, res) {
    ...
    if (isThenable.caught) {
        asyncFail(state, isThenable.value)
    } else if (isThenable.value) {
        Promise.resolve(res).then(
            function () { asyncFinish(state, tryPass()) },
            function (e) { asyncFail(state, e) })
    } else {
...
function asyncRun(state) {
    ...
    var res = try2(init, methods, methods, function (err) {
        asyncCallback(state, err)
    })
    ...
    if (res.caught) {
        ...
        if (state.timeout !== Infinity) {
            state.timer = setTimeout.call(global, function () {
                asyncFail(state, new Error(m("async.timeout", state.timeout)))
            }, state.timeout)
        }
    }
}
...
function invokeInit(test) {
    ...
    if (test.status & Flags.Inline) {
        ...
        start = now()
        attempt = try1(init, test.methods, test.methods)
    }
...
exports.async = function (methods, name, index, callback) {
    return createTest(methods, Flags.Async,
        new Data(name, index, methods._, {callback: callback}))
}
...
exports.syncInline = function (methods, name, index) {
    return createTest(methods, Flags.Init | Flags.Inline,
        new Data(name, index, methods._, {inline: []}))
}
