...
function Data(name, index, parent, callback) {
    this.name = name
    this.index = index
    this.parent = parent
...
function invokeAsync(test, then, res, start, resolve) { // eslint-disable-line max-params, max-len
    var resolved = false
    var timer
    ...
    var result = try2(then, res,
        function () { finish(tryPass()) },
        function (e) { finish(tryFail(addStack(e))) })
...
function invokeBlock(test) {
    var start = now()
    var tryBody = try1(test.data.callback, test.methods, test.methods)
    ...
    return new Promise(function (resolve) {
        invokeAsync(test, tryThen.value, tryBody.value, start, resolve)
    })
}
