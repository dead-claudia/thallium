...
methods(Report, {
    ...
    inspect: function () {
        ...
        return new function Report() {
            ...
            if (self.fail() || self.error() || self.extra()) {
...
function asyncRun(test, resolve) {
    var resolved = false
    var start = now()
    var res = try2(test.data.context, test.methods, test.methods)
    var timer

    function asyncFinish(attempt) {
        // Capture immediately. Worst case scenario, it gets thrown
        // away.
        var end = now()
    ...
    // Note: synchronous failures when initializing an async test are
    // test failures, not fatal errors.

    if (res.caught) {
        asyncFinish(tryFail(addStack(res.value)))
        return
    }

    var isThenable = try1(Resolver.isThenable, undefined, res.value)

    if (isThenable.caught) {
        asyncFinish(tryFail(addStack(isThenable.value)))
        return
    }
    ...
    // If an error was thrown from the start, don't set the timer.
    if (!resolved) {
        ...
        if (maxTimeout !== Infinity) {
            timer = setTimeout.call(global, function () {
                asyncFinish(tryFail(addStack(
                    new Error(m("async.timeout", maxTimeout)))))
            }, maxTimeout)
        }
    }
}
...
function invokeInit(test) {
    if (test.status & Flags.Async) {
        return new Promise(function (resolve) {
            asyncRun(test, resolve)
        })
    }
    ...
    if (test.status & Flags.Inline) {
        ...
        start = now()
        attempt = try1(test.data.context, test.methods, test.methods)
    }
...
exports.async = function (methods, name, index, callback) {
    return createTest(methods, Flags.Async,
        new Data(name, index, methods._, callback))
}
...
/**
 * Normal sync block tests, through `t.test()`.
 */
