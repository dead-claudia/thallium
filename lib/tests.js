...
var Flags = exports.Flags = Object.freeze({
    ...
    Skipped:   0x04, // If the test is explicitly skipped.
})
...
function BaseTest(methods, status, reporters, blocking, current) { // eslint-disable-line max-params, max-len
    ...
    // This is a pointer to the currently active test.
...
function Test(methods, status, reporters, blocking, name, index, parent, callback, current) { // eslint-disable-line max-params, max-len
    BaseTest.call(this, methods, status, reporters, blocking, current)
...
function Root(methods) {
    BaseTest.call(this, methods, Flags.Root, [], [], {value: this})
}
