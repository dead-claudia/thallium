...
var deprecate = require("../migrate/common.js").warn
...
function addTest(test, block, inline, name, callback) { // eslint-disable-line max-params, max-len
    ...
    if (test.status & Flags.Skipped || !isOnly(test, [name])) {
        ...
        if (callback != null) {
            ...
            // There's no easy way to move this into the `migrate` module,
            // because it's too engrained into core.
            deprecate("Inline tests are deprecated. Use block tests instead.")
