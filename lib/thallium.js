...
function attempt(func, a, b, c/* , ...args */) {
    ...
    if (this.status & Flags.Inline) {
        this.data.state.inline.push({func: func, args: args})
    } else {
...
methods(Reflect, {
    ...
    report: function (type, path, value, duration, slow) { // eslint-disable-line max-params, max-len
        ...
        if (internal !== Types.Fail &&
                internal !== Types.Error &&
                internal !== Types.Extra) {
            value = undefined
        }
    ...
    extra: function (count, value, stack) {
        ...
        return {count: count|0, value: value, stack: stack}
    },
...
require("./deprecated.js").coreProto(Thallium, Reflect, {
    checkInit: checkInit,
    isSkipped: isSkipped,
})
