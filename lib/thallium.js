...
var CoreCommon = require("./core/common.js")
...
methods(Reflect, {
    ...
    do: function (func) {
        ...
        if (!ApiUtil.isSkipped(this) && ApiUtil.isOnly(this._)) {
            if (this._.status & Flags.Inline) {
                this._.state.inline.push(new DelayedCall(func, []))
            } else {
                func()
            }
        }
    ...
    activeTimeout: function () {
        ApiUtil.checkInit(this._)
        return CoreCommon.timeout(this._)
    },
...
methods(Thallium, {
    ...
    run: function (callback) {
        ...
        .catch(function (e) {
            return CoreCommon.report(test, CoreCommon.r("error", e)).throw(e)
        })
        .bind().return()
        .asCallback(callback)
    },
