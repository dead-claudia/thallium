...
methods(Utils, {
    ...
    search: function (dir) {
        for (;;) {
            ...
            var iter = this.loaders.values()

            for (var next = iter.next(); !next.done; next = iter.next()) {
                // Skip the extension-free loaders
                if (next.value.ext) {
                    var file = base + next.value.ext
    ...
    findExact: function (list) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (LoaderData.isValid(glob)) {
                var parent = globParent(glob)
                var ext = LoaderData.getExt(glob)

                ret = this.walkUp(parent, ".tl" + ext)
