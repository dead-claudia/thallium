...
var getType = require("../util.js").getType
        ...
        if (Array.isArray(files)) {
            for (var i = 0; i < files.length; i++) {
                ...
                if (typeof pair !== "string") {
                    throw new TypeError(
                        "Expected config.files[" + i +
                        "] to be a string, but found a(n) " + getType(pair))
                }
            }
...
function mergeFiles(checked, files, baseDir, isDefault) {
    // Only merge if no files were explicitly specified on the command line.
    if (checked.files != null && isDefault) {
...
exports.merge = function (opts) {
    ...
    return Promise.try(function () {
        if (checked.thallium != null) {
            ...
            return checked.thallium
        } else {
            return opts.load("thallium", opts.baseDir).then(function (mod) {
                return exports.resolveDefault(mod.exports)
            })
        }
    })
    .then(function (thallium) {
        return {
            thallium: thallium,
            files: mergeFiles(
                checked,
