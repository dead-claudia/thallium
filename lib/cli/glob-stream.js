...
var globParent = require("./common.js").globParent
...
function addStream(cache, combined, streams, stream) {
    ...
    stream.on("data", /** @this */ function (chunk) {
        if (!hasOwn.call(cache, chunk.path)) {
            cache[chunk.path] = true
...
function globIsSingular(glob) {
    ...
    for (var i = 0; i < first.length; i++) {
        if (typeof first[i] !== "string") return false
    }
...
function streamFromPositive(negatives, positive, globOpts) {
    ...
    var base = globParent(glob) + path.sep

    // Create stream and map events from globber to it
    var stream

    if (negativeGlobs.length) {
        stream = through2.obj(null, function (file, enc, cb) {
            if (shouldInclude(negativeGlobs, file)) {
