 ...
 * also avoids Promises to avoid the dependency, and tries to read the files
 * first without `stat`ing them (it's faster to ask for forgiveness than
 * permission in this case). All this work to ensure this works as quickly as
 * possible.
 */
...
function findOpts(args, util, error, next) {
    /**
     * Lots of duplication here with `findConfig` from `./loader.js`, but I
    ...
    // Bluebird's `Promise.prototype.reduce` and `Array.prototype.reduce` both
    // won't work here, since one is stupid (it assumes `undefined` means use
    // the first entry, unlike most other `reduce` utilities) and the other
    // doesn't permit breaking early.
...
function loadOpts(args, util, error, next) {
    readOpts(args, util, error, function (dirname, contents) {
        if (dirname == null) return next()
        var extra = parse(splitOpts(contents))

        // Note: this list must be kept up with `Args` in `./parse.js`, or
    ...
    if (args.help) {
        ...
        loadOpts(args, Util, error, function () {
            if (args.respawn && args.unknown.length !== 0) {
                ...
                require("./run.js").run(args, Util).then(next, error)
            }
        })
    }
}
