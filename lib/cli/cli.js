...
function fixExtension(config, files) {
    if (!files.set) {
        var ext = config != null ? ".js" : path.extname(config)
...
methods(CliEngine, {
    ...
    requireConfig: function (next) {
        ...
        var result = config == null ? undefined : require(config)
    ...
    run: function () {
        ...
        var techtonic = loadRequires(this.args.cwd.value, this.data.requires)
        var self = this
