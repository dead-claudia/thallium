...
function keysToRegExp(object) {
    ...
    if (len === 0) return /(?!)/ // Matches nothing
    if (len === 1) return new RegExp(".(" + keys[0].replace(/\./, "\\.") + ")$")

    var items = ".(" + keys[0].replace(/\./, "\\.")

    for (var i = 0; i < keys.length; i++) {
        items += "|" + keys[i].replace(/\./g, "\\.")
    }
...
function isConfigLike(finder, base, map) {
    if (base.slice(0, 4) !== ".tl.") return false
    if (!finder.data.exts.test(base) && !interpretExt.test(base)) return false
    if (base.slice(-3) === ".js") return true
    return map.ext == null || base.slice(-map.ext.length) === map.ext
}

function higherPriority(finder, data, base) {
    // Check for the level before the index. The lower, the
    // higher. Also, plain JS is higher precedence than anything
    // else.
    return finder.last == null ||
        data.level < finder.last.data.level ||
        data.index < finder.last.data.index ||
        finder.last.base !== ".tl.js" && base === ".tl.js"
}
...
function checkDir(finder, dir) {
    ...
    .map(function (base) {
        ...
        var data = finder.dataMap[dir]

        // If we've already read a listing from it, it's clearly can't
        // be a config.
        if (!hasOwn.call(finder.cached, file) &&
                isConfigLike(finder, base, data)) {
            // Only do a stat if we have to. We can skip it if a last good file
            // exists, and this one is higher priority than that.

            if (finder.last == null) {
                return finder.state.util.stat(file).then(function (stat) {
                    if (higherPriority(finder, data, base)) {
                        if (stat.isFile()) {
                            finder.last = {
                                data: data,
                                base: base,
            ...
            } else if (higherPriority(finder, data, base)) {
                return finder.state.util.stat(file).then(function (stat) {
                    if (stat.isFile()) {
                        finder.last = {
                            data: data,
...
function findConfig(finder) {
    ...
    return Promise.map(Object.keys(finder.dataMap), function (dir) {
