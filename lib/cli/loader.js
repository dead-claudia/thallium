...
function loadDirect(loader) {
    assert(loader != null && typeof loader === "object")

    return Common.resolveDefault(
        loader.state.util.load(loader.mod, loader.baseDir))
}

function loadInterpret(loader) {
    assert(loader != null && typeof loader === "object")
    ...
    for (var i = 0; i < modules.length; i++) {
        ...
        try {
            if (typeof entry === "string") {
                loader.state.util.load(entry, loader.baseDir)
            } else {
                entry.register(
                    loader.state.util.load(entry.module, loader.baseDir))
            }
...
exports.load = function (loader) {
    ...
    if (!(loader.mask & Mask.Internal)) {
        return Promise.resolve(loader).then(loadDirect)
    } else if (loader.mask & Mask.Config) {
        return Promise.resolve()
    } else {
        return Promise.resolve(loader).then(loadInterpret)
    }
}
