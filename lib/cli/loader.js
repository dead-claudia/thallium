...
// Bluebird's `Promise.prototype.reduce` and `Array.prototype.reduce` both won't
// work here, since one is stupid (it assumes `undefined` means use the first
// entry, unlike most other `reduce` utilities) and the other doesn't permit
// breaking early.
    ...
    return findFirst(
        ...
        function (last, glob) {
            ...
            if (data.exts.test(glob)) {
                ...
                return searchPathExt(Common.globParent(glob), ext)
                .then(function (file) {
                    if (hasExt(file, ext)) {
                        last.matched = true
                        last.name = file
...
function loadExt(state, baseDir, mod) {
    ...
    function load(index) {
        ...
        .catch({code: "MODULE_NOT_FOUND"}, function () {
            return load(index + 1)
        })
    }
}
