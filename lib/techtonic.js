...
function matches(entry, item) {
    if (typeof entry === "string") {
        ...
        if (process.env.NODE_ENV === "development") {
            assert(entry instanceof RegExp)
        }
...
class Node {
    constructor(value) {
        if (process.env.NODE_ENV === "development") {
            if (value != null) {
                assert(typeof value === "string" || value instanceof RegExp,
                `value must be a string, RegExp, or null, but found ${value}`)
            }
        }
    ...
    addSingle(entry) {
        if (process.env.NODE_ENV === "development") {
            assert(typeof entry === "string" || entry instanceof RegExp,
                `entry must be a string, RegExp, or null, but found ${entry}`)
        }
