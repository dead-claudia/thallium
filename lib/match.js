...
/* global Buffer, Symbol, Uint8Array, DataView, ArrayBuffer, Map, Set */
...
var bufferSupport = (function () {
    ...
    if (new FakeBuffer().constructor !== FakeBuffer) return Safari
    if (typeof Buffer !== "function") return Polyfill
    if (typeof Buffer.isBuffer !== "function") return Polyfill
    // Avoid the polyfill
    if (Buffer.isBuffer(new FakeBuffer())) return Polyfill
    return Native
})()
...
function isBuffer(object) {
    if (bufferSupport === Native && Buffer.isBuffer(object)) return true
    if (bufferSupport === Safari && object._isBuffer) return true
...
// Setup for `isView`
var supportsArrayBuffer = typeof Uint8Array === "function" &&
...
var isBufferView = supportsArrayBuffer ? ArrayBuffer.isView : undefined
var isView = supportsArrayBuffer
    ...
    // does.
    ? function isView(a) {
        return isBuffer(a) || a instanceof ArrayBuffer || isBufferView(a)
    }
    : isBuffer
...
function matchMap(a, b, mask, left, right) { // eslint-disable-line max-params
    ...
    while (i !== end && match(akeys[i], bkeys[i], mask, left, right)) {
        i++
    }
    ...
    if (!matchKey(bkeys[i], akeys, i, end, mask, left, right)) {
        return false
    }
    ...
    while (++i < end) {
        ...
        // case.
        if (!match(key, akeys[i], mask, left, right) &&
                !matchKey(key, akeys, i, end, mask, left, right)) {
            return false
        }
    }
...
function matchSameProto(a, b, mask, left, right) { // eslint-disable-line max-params, max-len
    ...
    if (supportsArrayBuffer) {
        ...
        if (isBufferView(a)) return matchView(a, b)
    }
...
function matchInner(a, b, mask, left, right) { // eslint-disable-line max-params
    if (mask & SameProto) {
        if (Array.isArray(a)) return matchArrayLike(a, b, mask, left, right)

        if (supportsMap && a instanceof Map) {
            return matchMap(a, b, mask, left, right)
        }

        if (supportsSet && a instanceof Set) {
            return matchSet(a, b, mask, left, right)
        }
    }

    if (objectToString.call(a) === "[object Arguments]") {
        return matchArrayLike(a, b, mask, left, right)
    }
