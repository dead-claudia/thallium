...
var stackIncludesMessage = (function () {
    var stack = Errors.getStack(new Error("test"))
...
function getStack(e) {
    if (e instanceof Error) {
        ...
        if (stackIncludesMessage) {
            ...
            if (index < 0) return Errors.getStack(e).replace(/^\s+/gm, "")

            index = stack.indexOf(Console.newline, index + e.message.length)
            if (index >= 0) {
                stripped = stack.slice(index + Console.newline.length)
                    .replace(/^\s+/gm, "")
            }
        } else {
            stripped = Errors.getStack(e).replace(/^\s+/gm, "")
        }

        if (stripped !== "") description += Console.newline + stripped
        return description
    } else {
        return Errors.getStack(e).replace(/^\s+/gm, "")
    }
}
...
//     var lines = msg.split(Console.newline).slice(0, 4)
//     var ret = Console.newline + "      " +
...
//
//         if (line[0] === "+") {
//             ret += Console.newline + "      " + color("diff added", line)
//         } else if (line[0] === "-") {
//             ret += Console.newline + "      " + color("diff removed", line)
//         } else if (!/\@\@|\\ No newline/.test(line)) {
//             ret += Console.newline + "      " + line
//         }
//     }
//
//     return ret
