...
function getFullName(report) {
    var name = report._test.name
    var current = report._parent

    if (current == null || name == null) return undefined

    while (current._parent != null) {
        name = current._test.name + " " + name
...
function Path(test) {
    this._test = test
    this._parent = undefined // Set later
}
...
methods(Report, {
    ...
    get name() { return this._test.name },
    get index() { return this._test.index },
    get slow() { return this._test.slow || Constants.defaultSlow },
    get timeout() { return this._test.timeout || Constants.defaultTimeout },
    get isFailable() { return this._test.isFailable },

    get fullName() {
        return getFullName(
            this._type === Type.BeforeAll || this._type === Type.AfterAll
                ? this._parent
                : this
        )
    },
    ...
    get path() {
        var report = this // eslint-disable-line consistent-this
        var list = []
        var child

        if (this._type === Type.BeforeAll || this._type === Type.AfterAll) {
            report = report._parent
        }

        while (report._parent != null) {
            if (report._path == null) report._path = new Path(report._test)
            if (child != null) child._parent = report._path
            child = report._path
            list.push(child)
            report = report._parent
        }
