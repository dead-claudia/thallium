...
function getPath(test) {
    var ret = []
    var ctx = test

    while (!(ctx.status & Flags.Root)) {
        ret.push(Common.pathEntry(ctx.name, ctx.index))
        ctx = ctx.parent
    }
...
function runTest(test, isMain) {
    ...
    if (test.path == null) test.path = getPath(test)
