...
function Context(root, opts) {
    ...
    if (opts != null && opts.skip != null) {
        this.skip = Filter.create(opts.skip)
    }

    if (opts != null && opts.only != null) {
        this.only = Filter.create(opts.only)
    }
}
...
function runChildTests(test, context) {
    ...
    return peach(test.tests, function (child) {
        ...
        if (!isOnly(context)) {
            ...
            // Only explicitly skipped tests have no callback
            child.callback == null ||
            // Check agaist the `skip` option, too.
            context.skip != null && Filter.test(context.tests, context.skip)
        ) {
            test.root.current = child
