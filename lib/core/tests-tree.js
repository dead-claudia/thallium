...
function reportHook(context, test, errorWrap) {
    assert(context != null && typeof context === "object")
    ...
    case Stage.BeforeAll:
        return sendReport(context, Reports.beforeAll(test.data,
            errorWrap.data, errorWrap.error, errorWrap.func))

    case Stage.BeforeEach:
        return sendReport(context, Reports.beforeEach(test.data,
            errorWrap.data, errorWrap.error, errorWrap.func))

    case Stage.AfterEach:
        return sendReport(context, Reports.afterEach(test.data,
            errorWrap.data, errorWrap.error, errorWrap.func))

    case Stage.AfterAll:
        return sendReport(context, Reports.afterAll(test.data,
            errorWrap.data, errorWrap.error, errorWrap.func))
...
function invokeHook(test, list, stage) {
    ...
    return Util.peach(list, function (hook) {
        try {
            ...
            throw new ErrorWrap(test.data, stage, hook, e)
        }
    })
}
...
function runChildTests(context, test) {
    assert(test != null && typeof test === "object")
    assert(context != null && typeof context === "object")
    ...
    return Util.peach(test.tests, function (child) {
        ...
        if (!isOnly(context)) {
            ...
            return Util.pfinally(
                sendReport(context, Reports.skip(child.data)),
            ...
            .then(function () {
                ...
                child.data = Reports.data(child, test.data)
                context.tests.push(child)

                return Util.pfinally(
                    invokeBeforeEach(test)
                    .then(function () { return runNormalChild(context, child) })
                    .then(function () { return invokeAfterEach(test) })
                    .catch(function (e) {
                        if (!(e instanceof ErrorWrap)) throw e
                        return reportHook(context, test, e)
                    }),
                    function () {
                        test.root.current = test
                        Reports.deref(child.data)
                        context.tests.pop()
                    }
                )
            })
...
function runNormalChild(context, test) {
    assert(context != null && typeof context === "object")
    assert(test != null && typeof test === "object")
        ...
        .then(function (result) {
            if (result.caught) {
                if (result.value === Constants.Skip) {
                    return sendReport(context, Reports.skip(test.data))
                }
                if (!test.isFailable) context.isSuccess = false
                return sendReport(context,
                    Reports.fail(test.data, result.time, result.value))
            } else if (test.tests != null) {
                // Report this as if it was a parent test if it's passing and
                // has children.
                return sendReport(context,
                    Reports.enter(test.data, result.time))
                .then(function () {
                    return runChildTests(context, test)
                })
                .catch(function (e) {
                    if (!(e instanceof ErrorWrap)) throw e
                    return reportHook(context, test, e)
                ...
                .then(function () {
                    return sendReport(context, Reports.leave(test.data))
                })
            } else {
                return sendReport(context, Reports.pass(test.data, result.time))
            }
        }),
        function () { clearChildren(test) }
    )
...
exports.runTest = function (root, opts) {
    ...
    root.locked = true
    root.data = Reports.data(root)
    return Util.pfinally(
        sendReport(context, Reports.start(root.data))
        .then(function () { return runChildTests(context, root) })
        .catch(function (e) {
            if (!(e instanceof ErrorWrap)) throw e
            return reportHook(context, root, e)
        })
        .then(function () {
            return sendReport(context, Reports.end(root.data))
        })
        // Tell the reporter something happened. Otherwise, it'll have to wrap
        // this method in a plugin, which shouldn't be necessary.
        .catch(function (e) {
            return sendReport(context, Reports.error(e))
            .then(function () { throw e })
        }),
        function () {
            clearChildren(root)
            Reports.deref(root.data)
            root.locked = false
        }
    )
    .then(function () {
